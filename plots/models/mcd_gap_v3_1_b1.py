...
#
# Updates in v2_4:
#  --read observed spectra from files and overplot
#  --ad hoc Rayleigh scattering to guide the eye
#
# Updates in v3_0:
#  --change 'altered' disk region from fractional flux depression
#     to altered temp profile that corresponds to frac flux dep
#  --updated option for totally ad-hoc BS Rayleigh scattering curve (lam^-4)
#     done sloppily wrt coding--lots of sloppy stuff--fix later!!!
#
# Updates in v3_1:
#  --plot mult f_depress on same plot
#
####
#
...
if __name__ == "__main__":
    ...
    #   SUP=supress temp profile (compared to ZT) inside R_alt
    #       such that flux is down by factor f_depress
    #   If none of the above, default is ZT with warning
    #v3_1: changed tempmod to SUP
    tempmod='SUP'
    #plot 2nd temp mod
    tempmod2='NZT'
    #plot 3rd temp mod !!! this is bad programming, fix it
    tempmod3='SUP'
    #What fraction of flux, compared to unperturbed disk, is left in altered disk region?
    # !!!f_depress should depend on dotM; dotM in turn depends on h/R, alpha, c_s in disk!!!
    # !!!for now, just pick a number, but eventually make physical!!!
    # f_depress=1.0 is unperturbed disk; f_depress=0.0 is no disk in altered region
    #v3_1:
    #Changed default tempmod (usually ZT, unaltered disk) to be some other depression factor, and use suppressed temp model (f_depress2 here)
    f_depress=0.2
    f_depress2=0.01
    #
    ...
    ax1.set_xlabel(r"$Rest \ wavelength \ \lambda (nm)$")
    #Title is set for variable R_alt !!! fix this title so can make however many R_alt you want!!!
    #ax1.set_title(r"$Thermal \ Continuum; \ R_{alt}=%.1f, \ %.1f, \ %.1f \ r_{g}$" %(R_alt[0],R_alt[1],R_alt[2]))
    ax1.set_title(r"$Thermal \ Continuum; \ R_{sup}=%.1f \ r_{g}$" %R_alt[0])
    ...
    for j in range(len(R_alt)):
        ...
        #bad programming, see note at tempmod3!!!
        F_lam_3=np.zeros(len(lam))
        #compute temp, area, emitted spectrum at each radius; then sum
        for i in range((len(radius)-1)):
            #std
            #v3_1: not std anymore--now suppressed from R_alt, temp profile results in fluxes down by f_depress2
            Temp=find_Temp(epsilon,M_SMBH,dotm_edd,radius[i],radius_in,tempmod,R_alt[j],f_depress2)
            #comparison
            Temp_compare=find_Temp(epsilon,M_SMBH,dotm_edd,radius[i],radius_in,tempmod2,R_alt[j],f_depress)
            #3rd bad prog!!!
            Temp_3=find_Temp(epsilon,M_SMBH,dotm_edd,radius[i],radius_in,tempmod3,R_alt[j],f_depress)
            #std
            B_lambda=find_B_lambda(Temp, lam)
            #comparison
            B_lambda_compare=find_B_lambda(Temp_compare, lam)
            #3rd!!!
            B_lambda_3=find_B_lambda(Temp_3, lam)
            ...
            #comparison--compute annulus, but no suppression, add all in
            F_lam_ann_compare=PI*Area*B_lambda_compare
            F_lam_compare=F_lam_compare+F_lam_ann_compare
            #3rd!!!--compute annulus but suppression from T(r), add all in
            F_lam_ann_3=PI*Area*B_lambda_3
            F_lam_3=F_lam_3+F_lam_ann_3
            #std: construct altered disks using artificial suppression (maybe remove?)!!!
            if (radius[i] >= R_alt[j]):
    ...
    for j in range(len(R_alt)):
        ...
        #ax1.plot(lam/m_per_nm, flux_iter_R3[j]/normfactor, color=R_color_cycle[j], ls='dashed', linewidth=2, label=str(R_alt[j]))
        #v3_1: hardwired label
        ax1.plot(lam/m_per_nm, flux_iter_R3[j]/normfactor, color=R_color_cycle[j], ls='dashed', linewidth=2, label='sup=0.2')
    #plot unaltered spectrum (tempmod)
    #v3_1: hardwired label
    ax1.plot(lam/m_per_nm, F_lam_all/normfactor, color='black', ls='solid', linewidth=2, label='sup=0.01')
    ...
    for i in range(len(specfilenames)):
        ax1.plot(speclam[i]*speclamunits[i]/(1.0+redshift), specflux[i]*specfluxunits[i], color=speccolors[i], ls='solid', linewidth=1, label=specname[i])

    #!!!experiment with Rayleigh scattering (ad-hoc)
    #v3_1: commented out Rayleigh scattering expt
    #rayscat=23.0*pow(((lam/m_per_nm)/320.0),4.0)
    #ax1.plot(lam/m_per_nm,rayscat, color='b', ls='solid', linewidth=2, label='scattering')
    #legend
    ...
    savefig('mcd_gap_v3_1_b1.eps')
    savefig('mcd_gap_v3_1_b1.png')
    savefig('mcd_gap_v3_1_b1.pdf')
    show()
