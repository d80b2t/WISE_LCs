...
# R_alt    =        # outermost radius of altered disk; units of r_g of SMBH
...
if __name__ == "__main__":
    ...
    lam_min= 200.  #200.0  #357.
    lam_max= 800.  #800.0  #1029
    ...
    for j in range(len(R_alt)):
        ...
        myfile = open('xyz_temp.txt', 'w')
        for i in range((len(radius)-1)):

            myfile.write("R_alt= " + str(R_alt[j]) + "  i is " + str(i) + "   radius " + str(radius[i]) + "   lam " + str(lam) + " \n")
        ...
        print(j, 'i, F_lam_tot, F_lam_ann', i, F_lam_tot, F_lam_ann)
        myfile.close()
...
#ascii.write([speclam[0], specflux[0], log_F_lam_all, rayscat], 'asrt_temp.dat',            names=['speclam', 'specflux', 'log_F_lam_all', 'rayscat'], overwrite=True)
