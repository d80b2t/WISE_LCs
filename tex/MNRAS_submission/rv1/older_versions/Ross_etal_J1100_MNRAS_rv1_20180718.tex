...
\vspace{-16pt}
  \caption[]{
    ...
    model where the thermal disk emission interior to ${\bf 40 r_{\rm g}}$
...
depressing the UV emission. To check this scenario, we use the
standard assumptions from \citet{Whittet1992book} with  
$\rho_{\rm dust}$ being expressed in terms of $<A/L>$, extinction per kpc for 
the ISM: 
\begin{equation}
...
$\rho_{\rm dust} =2 \times 10^{-24}$ kg m$^{-1}$
and $\rho_{\rm gas}= 2 \times 10^{-22}$ kg m$^{-1}$, i.e. with 
areasonable gas-to-dust ratio of 100. 

In our case, $L$ is not kpc, but $<$0.01pc so there is a factor 10$^5$
increase in $<A/L>$ in order to produce the same level of extinction as 
the local ISM. This gives $\rho_{\rm dust} = 2 \times 10^{-19}$~kg~m$^{-3}$. 
If we increase the ISM density by an order of magnitude to get the \citep{Guo2016} 
result, $\rho_{\rm dust}= 2 \times 10^{-18}$~kg~m$^{-3}$, so 
$\rho_{\rm gas}=2 \times 10^{-16}$~kg~m$^{-3}$. Then
$M_{\rm cloud} = (4 \pi /3 \cdot  R_{\rm cloud}^{3}) \rho_{\rm gas}$  
which is 0.02 $M_{\odot}$ with $R_{\rm cloud}=0.01$ pc.

The extinction in \citet{Guo2016} is maybe an order of magnitude
larger than in \citet{Gordon2003}, i.e., 1.5 mag rather than 0.2
mag. From \citet{Gordon2003} we have $N_H \sim10^{21}$ cm$^{-2}$ for
an assumed gas/dust ratio $O(100)$. We need $N_H$ an order of
magnitude bigger, so $N_H~10^{22}$~cm$^{-2}$. If $R_{\rm cloud}=$ 0.01 pc,
then the number of particles of gas in the cloud is $N=N_H \cdot \pi
\cdot R_{\rm cloud}^2 = 3x10^{55}$ cm$^{-3}$ and $M_{\rm cloud} = n
\cdot m_{\rm H} =$ 0.05 $M_{\odot}$ with $R_{\rm cloud}=0.01$ pc to
contrive this.
%%
...
Guo et al. absorption as a piecewise 3-line fit to their extinction data in that their Figure 6. 
